...
I'm not the biggest fan of Stata, but I use it every day for RA work.
Though Stata shines like a beacon when all you want to do is explore one
data set (or a series of data sets that are easy to merge), whenever you
want to do something more complex or computationally intensive, it pales.
...
- Get a C compiler (`gcc` is standard and should be included Max and Linux; look into [MinGW](http://www.mingw.org) if you are on Windows to get `gcc`).
...
STDLL stata_call(int argc, char *argv[])
{
    ...
    // Alternatively, we can store the results in a macro and parse that
    // into a matrix char. After calling the plugin, run
    //
    // mata: strtoreal(tokens(st_local("b")))

     char buf[16], bchar[16 * b->size];
     strcpy (bchar, "");
     for (i = 0; i < b->size; i++) {
         sprintf (buf, " %15.9f", gsl_vector_get (b, i));
         strcat (bchar, buf);
     }
...
If you compile this into a plugin called `pluginSimci.plgin` then you would use
{% highlight stata %}
matrix b = J(10, 1, .)
sysuse auto
program pluginSimci, plugin using(./pluginSimci.plugin)
plugin call pluginSimci price mpg foreign, b 0.5 100000
mata: strtoreal(tokens(st_local("b")))'
matrix list b
{% endhighlight %}

To compile, on top of `gcc` and the SPI, you will need
- The [GNU Scientific Library (GSL)](https://www.gnu.org/software/gsl)
- [OpenMP](http://www.openmp.org)

Again, you should have `stplugin.c` and `stplugin.h` in the same directory. To compile on Linux/Unix, run
{% highlight bash %}
gcc -c -o stplugin.o    stplugin.c    -fopenmp -shared -fPIC -DSYSTEM=OPUNIX
gcc -c -o pluginSimci.o pluginSimci.c -fopenmp -shared -fPIC -DSYSTEM=OPUNIX
gcc -fopenmp -shared -fPIC -DSYSTEM=OPUNIX stplugin.o pluginSimci.o \
    -lgsl -lgslcblas -lm -o pluginSimci.plugin
{% endhighlight %}

See [Stata's documentation](http://www.stata.com/plugins) for instructions on how to build on other platforms.
